...
| `$releaseholds_tags` | array of globs | `('offsite-*')` | Tags to release when `$releaseholds='Y'`. Bash glob patterns; only matching holds are released. User holds that do not match block the snapshot deletion but do not fail the whole job. Set to `('*')` to restore the previous all-holds behavior |
| `$skipbusy`     | `'Y'` / `'N'` | `'Y'` in [zfsretain](../commands-and-modules/modules.md#zfsretain), `'N'` in direct `delsnap` | When `'Y'`, a `zfs destroy` failure (e.g. snapshot is busy or held) is logged as `WARN` and the script continues. When `'N'`, the failure is fatal. In both cases, [zfs-diagnose-busy](../commands-and-modules/modules.md#zfs-diagnose-busy) is called first to report the specific cause                                                                            |
...
| `$preserve_target_holds`        | `'Y'` = before a full copy (`doincrementals='N'`, or `force='Y'`), capture existing holds on the destination and reapply them after the transfer. Implemented in [zfs-send-receive](../commands-and-modules/modules.md#zfs-send-receive), so direct calls get it too; no-op on plain incremental runs and in dry-run mode. Default `'Y'` |
