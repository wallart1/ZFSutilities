...
| `test-zfscheckagainst` | 26 | `<offsite>` placeholder expansion in Source root and Destination root columns, offline hold-tag verification, literal counterparts, missing-snapshot fatal error, legacy `.conf` fallback |
