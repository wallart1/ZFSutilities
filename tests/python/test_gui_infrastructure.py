            ...
            for item in node.body:
                ...
                if isinstance(item, ast.Assign):
                    for target in item.targets:
                        if isinstance(target, ast.Name) and target.id == "_PAGE_ANCHORS":
                            value = item.value
                            break
                elif isinstance(item, ast.AnnAssign) and (
                    isinstance(item.target, ast.Name) and item.target.id == "_PAGE_ANCHORS"
                ):
                    value = item.value
...
class TestDocsViewerNavigation(unittest.TestCase):
    ...
    def test_tool_button_is_configured_for_toolbar(self):
        with mock_gtk() as gtk_mock:
            ...
            btn.get_style_context().add_class.assert_called_with("docs-toolbar-btn")
    ...
    def test_on_decide_policy_uses_navigation_action(self):
        with mock_gtk() as gtk_mock:
            ...
            result = win._on_decide_policy(
                win._webview, decision, WebKit2.PolicyDecisionType.NAVIGATION_ACTION
            )
    ...
    def test_on_decide_policy_non_navigation_returns_false(self):
        with mock_gtk() as gtk_mock:
            ...
            result = win._on_decide_policy(
                win._webview, decision, WebKit2.PolicyDecisionType.NEW_WINDOW_ACTION
            )
    ...
    def test_server_stops_and_releases_port(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                self.assertNotEqual(result, 0, "Server socket should be closed after stop()")
...
class TestUIStateManagerTreeviewColumns(unittest.TestCase):
    ...
    def test_do_save_collects_column_widths(self, mock_save):
        with mock_gtk() as gtk_mock:
            ...
            col1.get_fixed_width.return_value = 123
            col1.get_title.return_value = "Name"
            col2 = MagicMock()
            col2.get_resizable.return_value = False
            col2.get_fixed_width.return_value = 456
            col2.get_title.return_value = "Fixed"
            tv = MagicMock()
            ...
            self.assertEqual(args["treeview_columns"]["test_view"], {"Name": 123})
    ...
    @patch("backup_config.save_ui_state")
    def test_bind_treeview_does_not_scale_saved_widths_to_window(self, mock_save):
        ...
        with mock_gtk():
            ...
            col1.set_fixed_width.assert_called_once_with(200)
            col2.set_fixed_width.assert_called_once_with(300)
    ...
    def test_bind_treeview_defers_width_signal_until_after_restore(self, mock_save):
        ...
        with mock_gtk():
            ...
            set_width_indices = [i for i, c in enumerate(calls) if c[0] == "set_fixed_width"]
            notify_indices = [
                i for i, c in enumerate(calls) if c[0] == "connect" and c[2] == "notify::width"
            ]
    ...
    def test_bind_treeview_allows_scrolled_window_to_shrink(self, mock_save):
        ...
        with mock_gtk():
            ...
            tv.set_size_request.assert_called_once_with(gui_helpers.TREEVIEW_MIN_WIDTH, -1)
            scrolled.set_min_content_width.assert_called_once_with(gui_helpers.TREEVIEW_MIN_WIDTH)
    ...
    def test_do_save_skips_unrealized_treeview(self, mock_save):
        ...
        with mock_gtk() as gtk_mock:
            ...
            col1.get_fixed_width.return_value = 150
            col1.get_title.return_value = "Name"
            col2 = MagicMock()
            col2.get_resizable.return_value = True
            col2.get_min_width.return_value = 20
            col2.get_width.return_value = 200
            col2.get_fixed_width.return_value = 200
            col2.get_title.return_value = "Created"
            tv = MagicMock()
    ...
    def test_do_save_skips_placeholder_column_widths(self, mock_save):
        ...
        with mock_gtk() as gtk_mock:
            ...
            col1.get_resizable.return_value = True
            col1.get_min_width.return_value = 20
            col1.get_fixed_width.return_value = 0
            col1.get_title.return_value = "Name"
            col2 = MagicMock()
            col2.get_resizable.return_value = True
            col2.get_min_width.return_value = 20
            col2.get_fixed_width.return_value = 1
            col2.get_title.return_value = "Created"
            tv = MagicMock()
            tv.get_realized.return_value = True
            tv.get_columns.return_value = [col1, col2]
    ...
    def test_do_save_keeps_intentionally_minimized_columns(self, mock_save):
        ...
        with mock_gtk() as gtk_mock:
            ...
            col1.get_resizable.return_value = True
            col1.get_min_width.return_value = 20
            col1.get_fixed_width.return_value = 20
            col1.get_title.return_value = "Name"
            col2 = MagicMock()
            col2.get_resizable.return_value = True
            col2.get_min_width.return_value = 20
            col2.get_fixed_width.return_value = 20
            col2.get_title.return_value = "Created"
            tv = MagicMock()
            tv.get_realized.return_value = True
            tv.get_columns.return_value = [col1, col2]
            ...
            args = mock_save.call_args[0][1]
            self.assertIn("treeview_columns", args)
            self.assertEqual(args["treeview_columns"]["test_view"], {"Name": 20, "Created": 20})
    ...
    def test_bind_treeview_not_saved_before_idle_runs(self, mock_save):
        ...
        with mock_gtk() as gtk_mock:
            ...
            args = mock_save.call_args[0][1]
            self.assertIn("treeview_columns", args)
            self.assertEqual(args["treeview_columns"]["test_view"], {"Name": 150, "Created": 200})
    ...
    @patch("backup_config.save_ui_state")
    def test_do_save_persists_fixed_width_not_allocated_width(self, mock_save):
        """GTK may allocate narrower than fixed_width; persist fixed_width."""
        with mock_gtk() as gtk_mock:
            gtk_mock.TreeViewColumnSizing = MagicMock()
            gtk_mock.TreeViewColumnSizing.FIXED = 2
            import gui_helpers
            ...
            # User dragged the column to 200, but GTK allocated only 80.
            col.get_fixed_width.return_value = 200
            col.get_width.return_value = 80
            col.get_title.return_value = "Created"
            tv = MagicMock()
            tv.get_realized.return_value = True
            ...
            args = mock_save.call_args[0][1]
            self.assertIn("treeview_columns", args)
            self.assertEqual(args["treeview_columns"]["test_view"], {"Created": 200})

    @patch("backup_config.save_ui_state")
    def test_bind_treeview_restores_saved_widths_by_title(self, mock_save):
        ...
        with mock_gtk():
            ...
            win.popout_window = None
            config = {
                "ui_state": {"treeview_columns": {"test_view": {"Name": 150, "Created": 110}}}
            }
            mgr = gui_helpers.UIStateManager(win, config)
            ...
            col1.set_fixed_width.assert_called_once_with(150)
            col2.set_fixed_width.assert_called_once_with(110)

    @patch("backup_config.save_ui_state")
    def test_bind_treeview_skips_renamed_or_missing_columns(self, mock_save):
        ...
        with mock_gtk():
            ...
            win.vpaned.get_position.return_value = 0
            win.popout_window = None
            config = {"ui_state": {"treeview_columns": {"test_view": {"OldName": 150}}}}
            mgr = gui_helpers.UIStateManager(win, config)

            col = MagicMock()
            col.get_resizable.return_value = True
            col.get_min_width.return_value = 20
            col.get_title.return_value = "NewName"
            tv = MagicMock()
            tv.get_columns.return_value = [col]
...
class TestTreeviewColumnHelpers(unittest.TestCase):
    ...
    def test_configure_treeview_column_sets_fixed_width(self):
        with mock_gtk():
            ...
            col.set_sizing.assert_called_once_with(gui_helpers.Gtk.TreeViewColumnSizing.FIXED)
    ...
    def test_editable_list_view_default_column_names(self):
        with mock_gtk():
            import gui_helpers

            elv = gui_helpers.EditableListView()
            self.assertEqual(elv.column_names, ["source", "dest"])

    def test_editable_list_view_accepts_custom_column_names(self):
        with mock_gtk():
            import gui_helpers

            columns = [(1, "Source", 120), (2, "Destination", 120), (3, "Excludes", 120)]
            elv = gui_helpers.EditableListView(
                columns=columns, column_names=["source", "dest", "excludes"]
            )
            self.assertEqual(elv.column_names, ["source", "dest", "excludes"])

    def test_editable_list_view_rejects_mismatched_column_names(self):
        with mock_gtk():
            import gui_helpers

            with self.assertRaises(ValueError):
                gui_helpers.EditableListView(
                    columns=[(1, "Source", 120)],
                    column_names=["source", "dest"],
                )

    def test_editable_list_view_get_data_uses_column_names(self):
        with mock_gtk():
            import gui_helpers
            ...
            elv.store = _FakeStore()
            elv.set_data([(True, "a", "b", "c")])
            self.assertEqual(
                elv.get_data(),
                [{"active": True, "source": "a", "dest": "b", "excludes": "c"}],
            )
...
class TestTextViewSearchIcons(unittest.TestCase):
    ...
    def test_search_and_reset_are_icon_buttons(self):
        ...
        with mock_gtk() as gtk_mock:
            ...
            label_calls = [
                c
                for c in gtk_mock.Button.call_args_list
...
class TestAddVarRowEntryWidth(unittest.TestCase):
    ...
    def test_add_var_row_sets_width_chars_on_entry(self):
        ...
        with mock_gtk() as gtk_mock:
            ...
            grid = MagicMock()
            widgets = {}
            gui_helpers.add_var_row(grid, 0, "test_key", {"test_key": "value"}, widgets)
...
class TestAddVarRowYNCombo(unittest.TestCase):
    ...
    def _run(self, variables):
        ...
        with mock_gtk() as gtk_mock:
            ...
            grid = MagicMock()
            widgets = {}
            gui_helpers.add_var_row(grid, 0, "test_key", variables, widgets, yn_vars={"test_key"})
...
class TestPoolsControlsLayout(unittest.TestCase):
    ...
    def test_controls_rows_are_horizontal(self):
        ...
        with mock_gtk() as gtk_mock:
            ...
            horizontal_boxes = [b for b, kwargs in boxes if kwargs.get("orientation") == 1]
        ...
        for fname in os.listdir(pages_dir):
            ...
            for func in ast.walk(tree):
                ...
                if selectable_titles:
                    offenders.append(f"{fname}: {', '.join(sorted(selectable_titles))}")
...
class TestTreeSearchMatching(unittest.TestCase):
    ...
    def test_local_match_without_full_name_func(self):
        with mock_gtk():
            ...
            ts = gh.TreeSearch(MagicMock(), MagicMock(), MagicMock(), MagicMock(), MagicMock())
    ...
    def test_slash_match_with_full_name_func(self):
        with mock_gtk():
            ...
            self.assertEqual(ts._find_matches(store, "threeamigos/proxmox"), ["0:0"])
...
class TestTreeSearchGotoMatch(unittest.TestCase):
    ...
    def test_goto_match_expands_and_selects_path(self):
        with mock_gtk():
            ...
            with (
                patch.object(gh, "expand_path_to_row") as mock_expand,
    ...
    def test_update_matches_from_store_preserves_current_path(self):
        with mock_gtk():
            ...
            store = MagicMock()
            view.get_model.return_value = store
            ts = gh.TreeSearch(view, MagicMock(), MagicMock(), MagicMock(), MagicMock())
            ts._text = "foo"
            ts._matches = ["0:0"]
            ts._current_idx = 0
...
class TestHandleEditingKeyPress(unittest.TestCase):
    ...
    def _setup(self, gh, keyval, path_str, col_idx, editable_cols, has_next=True):
        widget = MagicMock()
        treeview = MagicMock()
        model = MagicMock()
    ...
    def test_tab_moves_to_next_editable_column(self):
        with mock_gtk():
            ...
            widget, treeview, _model, event = self._setup(gh, 1, "0", 0, [0, 1, 2])

            result = gh.handle_editing_key_press(widget, event, treeview, "0", 0, [0, 1, 2])
    ...
    def test_tab_at_last_editable_column_moves_to_next_row_first_column(self):
        with mock_gtk():
            ...
            widget, treeview, model, event = self._setup(gh, 1, "0", 2, [0, 1, 2], has_next=True)

            result = gh.handle_editing_key_press(widget, event, treeview, "0", 2, [0, 1, 2])
    ...
    def test_tab_at_last_column_last_row_does_not_advance(self):
        with mock_gtk():
            ...
            widget, treeview, _model, event = self._setup(gh, 1, "0", 2, [0, 1, 2], has_next=False)
    ...
    def test_shift_tab_moves_to_previous_editable_column(self):
        with mock_gtk():
            ...
            widget, treeview, _model, event = self._setup(gh, 2, "0", 1, [0, 1, 2])

            result = gh.handle_editing_key_press(widget, event, treeview, "0", 1, [0, 1, 2])
    ...
    def test_shift_tab_at_first_editable_column_moves_to_previous_row_last(self):
        with mock_gtk():
            ...
            widget, treeview, _model, event = self._setup(gh, 2, "1", 0, [0, 1, 2])

            result = gh.handle_editing_key_press(widget, event, treeview, "1", 0, [0, 1, 2])
    ...
    def test_shift_tab_at_first_row_first_column_does_not_advance(self):
        with mock_gtk():
            ...
            widget, treeview, _model, event = self._setup(gh, 2, "0", 0, [0, 1, 2])
