...
create_desktop_symlinks() {
    ...
    if [[ -z "${user}" ]]; then
        warn "  ⚠ Cannot determine desktop user; skipping home-directory symlinks."
        log_msg "INFO:     To create them manually, run:"
        log_msg "INFO:       ln -s /usr/local/lib/zfsutilities/current/bin/zfsutilities-gui" \
    ...
    log_msg "INFO:   ✓ ${gui_link} -> ${gui_target}"

    rm -f "${docs_link}"
    ln -s "${docs_target}" "${docs_link}"
    chown -h "${user}:" "${docs_link}" 2>/dev/null || true
    log_msg "INFO:   ✓ ${docs_link} -> ${docs_target}"
